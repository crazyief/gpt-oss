...
function createMessagesStore() {
	...
	return {
		 ...
		 *
		 * WHY merge streamingContent into messageData:
		 * - Backend may not have sent content in complete event (depends on implementation)
		 * - streamingContent is authoritative for what user saw during streaming
		 * - Ensures displayed content matches what was streamed
		 *
		...
		finishStreaming: (messageData: Message) => {
			update((state) => ({
				...
				items: [
					...
					{
						...messageData,
						// Use streamingContent as content if messageData.content is empty
						content: messageData.content || state.streamingContent
					}
				]
			}));
		},
