 ...
 *
 * Flow:
 * 1. Call API to create conversation in currently selected project
 * 2. Add conversation to store
 * 3. Switch to new conversation
 * 4. Clear message history (fresh start)
 * 5. Notify parent component
 *
 * WHY use currentProjectId from store:
 * - Project awareness: Creates conversation in selected project
 * - Default behavior: If "All Projects" selected ($currentProjectId = null), creates in default project
 * - User expectation: New conversations should belong to current context
 *
...
<style>
	 ...
	 *
	 * WHY calc(100% - 1.5rem) width:
	 * - Prevents overflow: Accounts for 0.75rem margin on both sides
	 * - No collision: Button stays within sidebar boundaries
	 */
	.new-chat-button {
		display: flex;
		align-items: center;
