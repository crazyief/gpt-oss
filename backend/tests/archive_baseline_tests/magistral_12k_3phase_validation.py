...
MODEL_NAME = "Magistral-Small-2506-Q6_K_L (12k context)"
...
def phase1_coarse_discovery():
    ...
    output_file = f"magistral_12k_phase1_{timestamp}.json"
...
def phase3_production_validation():
    ...
    # Save results
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = f"magistral_12k_phase3_{timestamp}.json"
...
def generate_markdown_report(phase1_results, phase3_results):
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = f"magistral_12k_validation_results_{timestamp}.md"
...
def run_full_3phase_validation():
    ...
    print("\n" + "=" * 80)
    print(f"MAGISTRAL-SMALL-2506 12K CONTEXT - 3-PHASE VALIDATION")
    print("=" * 80)
    print(f"Model: {MODEL_NAME}")
