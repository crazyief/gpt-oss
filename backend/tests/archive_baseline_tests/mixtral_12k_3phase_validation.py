...
MODEL_NAME = "mixtral-8x7b-v0.1.Q3_K_M (12k context)"
...
def phase1_coarse_discovery():
    ...
    output_file = f"mixtral_12k_phase1_{timestamp}.json"
...
def phase3_production_validation():
    ...
    # Save results
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = f"mixtral_12k_phase3_{timestamp}.json"
...
def generate_markdown_report(phase1_results, phase3_results):
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = f"mixtral_12k_validation_results_{timestamp}.md"
...
def run_full_3phase_validation():
    ...
    print("\n" + "=" * 80)
    print(f"MIXTRAL-8X7B 12K CONTEXT - 3-PHASE VALIDATION")
    print("=" * 80)
    print(f"Model: {MODEL_NAME}")
