...
MODEL_NAME = "Magistral-Small-2506-Q8_0 (16k context)"
...
def phase1_coarse_discovery():
    ...
    output_file = f"magistral_q8_16k_phase1_{timestamp}.json"
...
def analyze_phase1_results(results: list):
    ...
    print("-" * 80)
    if all_perfect:
        print("[OK] All tests passed! Safe zone confirmed (50-950 items).")
    else:
...
def phase3_production_validation():
    ...
    # Save results
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = f"magistral_q8_16k_phase3_{timestamp}.json"
...
def generate_markdown_report(phase1_results, phase3_results):
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = f"magistral_q8_16k_validation_results_{timestamp}.md"

    with open(output_file, "w", encoding="utf-8") as f:
        f.write(f"# phi-4-reasoning 12k Context Validation Report\n\n")
        f.write(f"**Model**: {MODEL_NAME}\n")
        f.write(f"**Context Window**: 16,000 tokens (~600 items)\n")
        ...
        # Phase 1 Summary
        f.write("## Phase 1: Coarse Discovery (50-950 items)\n\n")
        ...
        if all_perfect:
            ...
            f.write("- Safe zone confirmed: 50-950 items\n")
...
def run_full_3phase_validation():
    ...
    print("\n" + "=" * 80)
    print(f"MAGISTRAL-Q8_0 16K CONTEXT - 3-PHASE VALIDATION")
    print("=" * 80)
    print(f"Model: {MODEL_NAME}")
    print(f"Context Window: 16,000 tokens")
    print(f"Safe Zone Target: 50-950 items")
