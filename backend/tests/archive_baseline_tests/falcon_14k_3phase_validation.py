"""
Falcon-H1-34B @ 14k Context - 3-Phase Validation
...
MODEL_NAME = "Falcon-H1-34B-Instruct-Q5_K_M (14k context)"
...
def phase1_coarse_discovery():
    ...
    output_file = f"falcon_14k_phase1_{timestamp}.json"
...
def phase3_production_validation():
    ...
    # Save results
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = f"falcon_14k_phase3_{timestamp}.json"
...
def generate_markdown_report(phase1_results, phase3_results):
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = f"falcon_14k_validation_results_{timestamp}.md"

    with open(output_file, "w", encoding="utf-8") as f:
        f.write(f"# Falcon-H1-34B 14k Context Validation Report\n\n")
        f.write(f"**Model**: {MODEL_NAME}\n")
        f.write(f"**Context Window**: 14,000 tokens (~800 items)\n")
