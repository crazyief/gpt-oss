...
# -*- coding: utf-8 -*-
"""
Gemma 27B Scientific Validation Test
Automatically runs 5 iterations to align with Magistral-2506 test points
"""
...
def test_position(num_items, position_name, index):
    ...
    try:
        response = requests.post(
            "http://localhost:8090/v1/chat/completions",  # Gemma port
            json={
...
def main():
    print("="*70)
    print("[TEST] GEMMA 27B SCIENTIFIC VALIDATION TEST")
    print(f"[TIME] Started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("="*70)

    print("\n[PLAN] TEST PLAN:")
    print("   - 250 items  (Missing data point - align with Magistral)")
    print("   - 750 items  (Missing data point - align with Magistral)")
    print("   - 1000 items (Missing data point - align with Magistral)")
    print("   - 1250 items (Validate existing 67% reading)")
    print("   - 1400 items (Missing data point - align with Magistral)")
    print("\n   Each test: 5 runs x 3 positions = 15 total queries")
    print("   Total: 75 queries across all test sizes")
    print("   Estimated time: ~2-3 hours\n")

    gemma_tests = {
        250: "Missing data point - align with Magistral",
        750: "Missing data point - align with Magistral",
        1000: "Missing data point - align with Magistral",
        1250: "Validate existing 67% reading",
        1400: "Missing data point - align with Magistral"
    }

    all_results = {}
    test_num = 1
    total_tests = len(gemma_tests)

    for num_items, description in gemma_tests.items():
        print(f"\n{'+'+'='*68+'+'}")
        ...
        if num_items != list(gemma_tests.keys())[-1]:
            print("\n" + "[WAIT] "*35)
    ...
    filename = f"gemma_validation_results_{timestamp}.md"

    with open(filename, "w", encoding="utf-8") as f:
        f.write(f"# Gemma 27B Scientific Validation Results\n\n")
        f.write(f"**Date**: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"**Purpose**: Fill missing data points to align with Magistral-2506 test coverage\n\n")
        ...
        f.write("\n## Alignment with Magistral-2506\n\n")
        f.write("Test points now match Magistral-2506 coverage:\n")
        f.write("- 250 items: ✅ Validated\n")
        f.write("- 750 items: ✅ Validated\n")
        f.write("- 1000 items: ✅ Validated\n")
        f.write("- 1250 items: ✅ Validated\n")
        f.write("- 1400 items: ✅ Validated\n\n")
        f.write("This allows direct comparison across all three models.\n")
    ...
    for num_items, data in all_results.items():
        ...
        if middle_acc == 1.0:
            print(f"[OK] {num_items} items: 100% accuracy (all positions work)")
        elif middle_acc == 0.0:
            print(f"[FAIL] {num_items} items: 0% middle accuracy (systematic bug)")
        elif middle_acc >= 0.8:
            print(f"[WARN] {num_items} items: {middle_acc:.0%} (mostly reliable)")
        elif middle_acc >= 0.4:
            print(f"[WARN] {num_items} items: {middle_acc:.0%} (inconsistent)")
        else:
            print(f"[FAIL] {num_items} items: {middle_acc:.0%} (mostly fails)")
