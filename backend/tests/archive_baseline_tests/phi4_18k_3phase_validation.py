...
MODEL_NAME = "phi-4-reasoning-UD-Q8_K_XL (18k context)"
...
def phase1_coarse_discovery():
    ...
    output_file = f"phi4_18k_phase1_{timestamp}.json"
...
def phase3_production_validation():
    ...
    # Save results
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = f"phi4_18k_phase3_{timestamp}.json"
...
def generate_markdown_report(phase1_results, phase3_results):
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = f"phi4_18k_validation_results_{timestamp}.md"
