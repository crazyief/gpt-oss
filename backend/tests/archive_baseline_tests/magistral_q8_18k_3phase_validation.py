...
MODEL_NAME = "Magistral-Small-2506-Q8_0 (18k context)"
...
def phase1_coarse_discovery():
    ...
    output_file = f"magistral_q8_18k_phase1_{timestamp}.json"
...
def phase3_production_validation():
    ...
    # Save results
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = f"magistral_q8_18k_phase3_{timestamp}.json"
...
def generate_markdown_report(phase1_results, phase3_results):
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = f"magistral_q8_18k_validation_results_{timestamp}.md"

    with open(output_file, "w", encoding="utf-8") as f:
        f.write(f"# phi-4-reasoning 12k Context Validation Report\n\n")
        f.write(f"**Model**: {MODEL_NAME}\n")
        f.write(f"**Context Window**: 18,000 tokens (~600 items)\n")
...
def run_full_3phase_validation():
    ...
    print("\n" + "=" * 80)
    print(f"MAGISTRAL-Q8_0 18K CONTEXT - 3-PHASE VALIDATION")
    print("=" * 80)
    print(f"Model: {MODEL_NAME}")
