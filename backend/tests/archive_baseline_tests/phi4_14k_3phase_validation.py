...
MODEL_NAME = "phi-4-reasoning-UD-Q8_K_XL (14k context)"
...
def phase1_coarse_discovery():
    ...
    output_file = f"phi4_14k_phase1_{timestamp}.json"
...
def phase3_production_validation():
    ...
    # Save results
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = f"phi4_14k_phase3_{timestamp}.json"
...
def generate_markdown_report(phase1_results, phase3_results):
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = f"phi4_14k_validation_results_{timestamp}.md"
...
def run_full_3phase_validation():
    ...
    print("\n" + "=" * 80)
    print(f"PHI-4-REASONING 14K CONTEXT - 3-PHASE VALIDATION")
    print("=" * 80)
    print(f"Model: {MODEL_NAME}")
