...
MODEL_NAME = "Magistral-Small-2506-Q6_K_L (20k context)"
...
def phase1_coarse_discovery():
    ...
    output_file = f"magistral_20k_phase1_{timestamp}.json"
...
def phase3_production_validation():
    ...
    # Save results
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = f"magistral_20k_phase3_{timestamp}.json"
...
def generate_markdown_report(phase1_results, phase3_results):
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = f"magistral_20k_validation_results_{timestamp}.md"
