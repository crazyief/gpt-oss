#!/usr/bin/env python3
"""
Context Limit Test for Gemma 3 27B
Testing real-world context handling vs theoretical 131k limit
"""
    ...
    for test_idx, position in test_positions:
        ...
        try:
            response = requests.post(
                ...
                timeout=60  # Longer timeout for Gemma
            )
...
def main():
    """Run context limit test for Gemma"""

    print("="*60)
    print("CONTEXT LIMIT TEST FOR GEMMA 3 27B")
    print(f"Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("Model: google_gemma-3-27b-it-qat-Q6_K")
    print("Theoretical context: 131,072 tokens")
    print("="*60)

    # Test sizes - going much larger than Mistral
    # Gemma should handle much more with its 131k context
    # Testing up to ~6000 items (should be ~130k tokens)
    test_sizes = [
        100, 250, 500, 750, 1000, 1250, 1500, 1750, 2000,
        2250, 2500, 2750, 3000, 3500, 4000, 4500, 5000, 5500, 6000
    ]
    ...
    for size in test_sizes:
        ...
        # Wait between sizes (shorter for smaller sizes)
        wait_time = 5 if size < 2000 else 10
    ...
    # Final report
    print("\n" + "="*60)
    print("GEMMA 3 27B - FINAL RESULTS")
    print("="*60)
    ...
    if context_limit_hit:
        ...
        if last_working and last_working['status'] == "OK":
            ...
            print(f"  - Theoretical limit: 131,072 tokens")
            print(f"  - Usage: {last_working['tokens']/131072:.1%} of theoretical")
    ...
    print("\n" + "="*60)
    print("COMPARISON: GEMMA vs MISTRAL")
    print("="*60)
    print("\nMistral Small 24B (Previous Test):")
    print("  - Maximum items: 1,400")
    print("  - Maximum tokens: 31,710")
    print("  - Context limit: 32,768 tokens")

    if max_reliable > 0:
        print(f"\nGemma 3 27B (This Test):")
        print(f"  - Maximum items: {max_reliable:,}")
        print(f"  - Maximum tokens: {max_r['tokens']:,}")
        print(f"  - Context limit: 131,072 tokens")
        print(f"\nGemma advantage: {max_reliable/1400:.1f}x more items than Mistral")

    # Save detailed results
    with open("gemma_context_results.md", "w", encoding="utf-8") as f:
        f.write(f"# Gemma 3 27B Context Limit Test\n\n")
        f.write(f"**Date**: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"**Model**: google_gemma-3-27b-it-qat-Q6_K\n")
        f.write(f"**Theoretical Context**: 131,072 tokens\n\n")
        ...
        f.write("| Model | Max Items | Max Tokens | Context Limit | Advantage |\n")
        f.write("|-------|-----------|------------|---------------|----------|\n")
        f.write(f"| Mistral 24B Q6_K | 1,400 | 31,710 | 32,768 | Baseline |\n")
        if max_reliable > 0:
            f.write(f"| Gemma 3 27B | {max_reliable:,} | {max_r['tokens']:,} | 131,072 | {max_reliable/1400:.1f}x |\n")

    print(f"\nDetailed results saved to: gemma_context_results.md")
