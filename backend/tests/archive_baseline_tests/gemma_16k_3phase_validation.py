...
MODEL_NAME = "google_gemma-3-27b-it-qat-Q6_K (16k context)"
...
def phase1_coarse_discovery():
    ...
    output_file = f"gemma_16k_phase1_{timestamp}.json"
...
def phase3_production_validation():
    ...
    # Save results
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = f"gemma_16k_phase3_{timestamp}.json"
...
def generate_markdown_report(phase1_results, phase3_results):
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = f"gemma_16k_validation_results_{timestamp}.md"
...
def run_full_3phase_validation():
    ...
    print("\n" + "=" * 80)
    print(f"GEMMA-3-27B 16K CONTEXT - 3-PHASE VALIDATION")
    print("=" * 80)
    print(f"Model: {MODEL_NAME}")
