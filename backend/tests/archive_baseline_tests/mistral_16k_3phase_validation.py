"""
Falcon-H1_Q5 13k Context - 3-Phase Validation
Validate safe zone (50-650 items) with full methodology
"""
...
MODEL_NAME = "Mistral-Small-24B-2501-Q6_K (16k context)"
...
def phase1_coarse_discovery():
    ...
    for n_items in range(50, 851, 50):
    ...
    output_file = f"mistral_16k_phase1_{timestamp}.json"

    with open(output_file, "w", encoding="utf-8") as f:
        json.dump({
            "model": MODEL_NAME,
            "context_window": "13k tokens",
            "phase": 1,
            "parameters": {
                "start": 50,
                "end": 650,
...
def phase3_production_validation():
    ...
    - Test critical boundaries: 50, 300, 600, 650 items
    - 5 runs per item count (95% confidence)
    """
    print("\n" + "=" * 80)
    print("PHASE 3: PRODUCTION VALIDATION")
    print("=" * 80)
    print(f"Validation points: 50, 300, 600, 800 items")
    print(f"Runs per item count: 5")
    print(f"Confidence target: 95%")
    print(f"Estimated queries: {4 * 5 * 3} = 60")
    print(f"Estimated time: ~3-4 minutes")
    print("=" * 80)

    results = []
    positions = ["first", "middle", "last"]
    test_points = [50, 300, 600, 800]
    ...
    # Save results
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = f"mistral_16k_phase3_{timestamp}.json"
...
def generate_markdown_report(phase1_results, phase3_results):
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = f"backend/tests/falcon_13k_validation_results_{timestamp}.md"

    with open(output_file, "w", encoding="utf-8") as f:
        f.write(f"# Falcon-H1_Q5 13k Context Validation Report\n\n")
        f.write(f"**Model**: {MODEL_NAME}\n")
        f.write(f"**Context Window**: 13,000 tokens (~650 items)\n")
        ...
        if all_perfect:
            ...
            f.write("- Production-ready with 13k context configuration\n")
        else:
...
def run_full_3phase_validation():
    ...
    print(f"Safe Zone Target: 50-800 items")
