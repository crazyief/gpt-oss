"""
PHI-4-REASONING-PLUS 12K CONTEXT VALIDATION
Official ChatML Format - Based on Ultra-AI Agent Recommendation
"""
...
def run_phase1():
    ...
    for n_items in PHASE1_ITEM_RANGE:
        ...
        for run in range(1, PHASE1_RUNS_PER_ITEM + 1):
            for position in PHASE1_POSITIONS:
                ...
                context, secret = build_context(n_items, position)
    ...
    phase1_file = f"phi4_14k_phase1_{timestamp}.json"
...
def run_phase3(safe_zone: int):
    ...
    # IMMEDIATE SAVE with progress logging
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    phase3_file = f"phi4_14k_phase3_{timestamp}.json"
...
def main():
    ...
    print("=" * 80)
    print("PHI-4-REASONING-PLUS ADAPTIVE VALIDATION - 12K CONTEXT")
    print("=" * 80)
