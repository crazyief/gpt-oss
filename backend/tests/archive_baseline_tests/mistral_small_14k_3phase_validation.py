"""
phi-4-reasoning @ 14k Context - 3-Phase Validation
...
MODEL_NAME = "Mistral-Small-24B-Instruct-2501-Q6_K (14k context)"
...
def phase1_coarse_discovery():
    ...
    for n_items in range(50, 801, 50):
    ...
    output_file = f"mistral_small_14k_phase1_{timestamp}.json"
...
def analyze_phase1_results(results: list):
    ...
    print("-" * 80)
    if all_perfect:
        print("[OK] All tests passed! Safe zone confirmed (50-750 items).")
    else:
...
def phase3_production_validation():
    ...
    - Test critical boundaries: 50, 350, 700 items
    - 5 runs per item count (95% confidence)
    """
    print("\n" + "=" * 80)
    print("PHASE 3: PRODUCTION VALIDATION")
    print("=" * 80)
    print(f"Validation points: 50, 350, 700 items")
    ...
    # Save results
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = f"mistral_small_14k_phase3_{timestamp}.json"
...
def generate_markdown_report(phase1_results, phase3_results):
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = f"mistral_small_14k_validation_results_{timestamp}.md"

    with open(output_file, "w", encoding="utf-8") as f:
        f.write(f"# phi-4-reasoning 14k Context Validation Report\n\n")
        f.write(f"**Model**: {MODEL_NAME}\n")
        f.write(f"**Context Window**: 14,000 tokens (~700 items)\n")
        ...
        # Phase 1 Summary
        f.write("## Phase 1: Coarse Discovery (50-750 items)\n\n")
        ...
        if all_perfect:
            ...
            f.write("- Safe zone confirmed: 50-750 items\n")
            f.write("- 100% accuracy across all positions\n")
            f.write("- Production-ready with 14k context configuration\n")
        else:
...
def run_full_3phase_validation():
    ...
    print("\n" + "=" * 80)
    print(f"MISTRAL-SMALL-24B 14K CONTEXT - 3-PHASE VALIDATION")
    print("=" * 80)
    print(f"Model: {MODEL_NAME}")
    print(f"Context Window: 14,000 tokens")
    print(f"Safe Zone Target: 50-750 items")
