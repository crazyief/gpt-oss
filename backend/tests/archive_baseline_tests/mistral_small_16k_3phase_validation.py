"""
phi-4-reasoning @ 16k Context - 3-Phase Validation
...
MODEL_NAME = "Mistral-Small-24B-Instruct-2501-Q6_K (16k context)"
...
def phase1_coarse_discovery():
    ...
    print(f"Estimated queries: {18 * 3 * 3} = 162")
    ...
    for n_items in range(50, 951, 50):
    ...
    output_file = f"mistral_small_16k_phase1_{timestamp}.json"
...
def analyze_phase1_results(results: list):
    ...
    print("-" * 80)
    if all_perfect:
        print("[OK] All tests passed! Safe zone confirmed (50-900 items).")
    else:
...
def phase3_production_validation():
    ...
    # Save results
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = f"mistral_small_16k_phase3_{timestamp}.json"
...
def generate_markdown_report(phase1_results, phase3_results):
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = f"mistral_small_16k_validation_results_{timestamp}.md"

    with open(output_file, "w", encoding="utf-8") as f:
        f.write(f"# phi-4-reasoning 16k Context Validation Report\n\n")
        f.write(f"**Model**: {MODEL_NAME}\n")
        f.write(f"**Context Window**: 16,000 tokens (~800 items)\n")
        ...
        # Phase 1 Summary
        f.write("## Phase 1: Coarse Discovery (50-900 items)\n\n")
        ...
        if all_perfect:
            ...
            f.write("- Safe zone confirmed: 50-900 items\n")
            f.write("- 100% accuracy across all positions\n")
            f.write("- Production-ready with 16k context configuration\n")
        else:
...
def run_full_3phase_validation():
    ...
    print("\n" + "=" * 80)
    print(f"MISTRAL-SMALL-24B 16K CONTEXT - 3-PHASE VALIDATION")
    print("=" * 80)
    print(f"Model: {MODEL_NAME}")
    print(f"Context Window: 16,000 tokens")
    print(f"Safe Zone Target: 50-900 items")
