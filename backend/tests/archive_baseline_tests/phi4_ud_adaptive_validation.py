"""
PHI-4-REASONING-UD-Q8_K_XL Context Validation
==============================================
Enhanced 3-Phase Validation with Thermal Protection and Cryptographic Secrets

Features:
- Cryptographically secure random numbers (anti-pattern validation)
...
MODEL_NAME = "phi-4-reasoning-UD-Q8_K_XL"
LLM_API_URL = "http://localhost:8090/completion"

# Thermal Management
THERMAL_CONFIG = {
    "query_delay": 3,              # 3s between each query
    "batch_delay": 45,             # 45s after every 10 queries
    "phase_delay": 120,            # 2 min between Phase 1 and Phase 3
    "context_delay": 300,          # 5 min between context tests
...
def get_gpu_temperature() -> int:
    ...
    try:
        ...
        print(f"⚠️  Cannot read GPU temp: {e}")
        return None
...
def adaptive_thermal_throttling() -> Tuple[int, str]:
    ...
    if temp >= THERMAL_CONFIG["temp_threshold_abort"]:
        msg = f"🔴 CRITICAL: {temp}°C / {power}W - Emergency 5 min cooling"
        print(msg)
        return 300, msg
    elif temp >= THERMAL_CONFIG["temp_threshold_pause"]:
        msg = f"🟠 HOT: {temp}°C / {power}W - Extended 2 min cooling"
        print(msg)
        return 120, msg
    elif temp >= 75:
        msg = f"🟡 WARM: {temp}°C / {power}W - Brief 45s cooling"
        print(msg)
        return 45, msg
    else:
        msg = f"🟢 NORMAL: {temp}°C / {power}W"
        return 0, msg
...
def log_temperature_with_cooling(force_check: bool = False):
    ...
    if force_check or query_count % THERMAL_CONFIG["temp_check_interval"] == 0:
        ...
        if cooling_time > 0:
            print(f"    ⏸️  Cooling for {cooling_time}s...")
            time.sleep(cooling_time)
            # Re-check after cooling
            temp = get_gpu_temperature()
            print(f"    ✓ Resumed - GPU now: {temp}°C")
...
def generate_filler_items(n_items: int, secret_number: str, needle_position: str) -> str:
    ...
    for i in range(1, n_items + 1):
        ...
        if needle_position == "first" and i == max(1, int(n_items * 0.05)):
            items.append(f"Item {i}: The secret number is {secret_number}")
    ...
    if not secret_inserted:
        if needle_position == "last":
            items[-1] = f"Item {n_items}: The secret number is {secret_number}"
        else:
            items[0] = f"Item 1: The secret number is {secret_number}"
...
def query_llm(n_items: int, secret_number: str, needle_position: str,
    ...
    """
    Query the LLM with needle-in-haystack test.

    Returns:
        (success, model_answer, response_time)
    """
    global query_count
    query_count += 1
    ...
    prompt = f"""Below is a list of items. Find and return ONLY the 6-digit secret number mentioned in one of the items.

{context}

What is the secret number? Return ONLY the 6-digit number, nothing else."""
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if numbers:
                answer = numbers[0]
                success = (answer == secret_number)
                return success, answer, elapsed
            else:
                return False, content, elapsed
        else:
...
def phase1_coarse_discovery(context_size: int, start_items: int = 50,
    ...
    for n_items in range(start_items, max_items + 1, step):
        ...
        if accuracy == 0:
            ...
            if consecutive_failures >= 3:
                print(f"\n⚠️  Stopping: 3 consecutive failures detected")
                break
        else:
            consecutive_failures = 0
        ...
        if n_items % (step * 2) == 0:
            print(f"    🔄 Batch cooling: {THERMAL_CONFIG['batch_delay']}s")
    ...
    print(f"\n⏸️  Cooling {THERMAL_CONFIG['phase_delay']}s before Phase 3...")
...
def run_adaptive_validation(context_size: int):
    ...
    print("\n" + "="*80)
    print(f"PHI-4-UD-Q8_K_XL ADAPTIVE VALIDATION - {context_size//1000}K CONTEXT")
    print("="*80)
    ...
    # Save Phase 1
    phase1_file = f"phi4_ud_{context_size//1000}k_phase1_{timestamp}.json"
    with open(phase1_file, 'w') as f:
        json.dump(phase1_results, f, indent=2)

    # Save Phase 3
    phase3_file = f"phi4_ud_{context_size//1000}k_phase3_{timestamp}.json"
    with open(phase3_file, 'w') as f:
        json.dump(phase3_results, f, indent=2)

    # Save thermal log
    thermal_file = f"phi4_ud_{context_size//1000}k_thermal_{timestamp}.json"
    with open(thermal_file, 'w') as f:
        json.dump(temp_log, f, indent=2)

    # Generate markdown report
    report_file = f"phi4_ud_{context_size//1000}k_validation_results_{timestamp}.md"
    ...
    print(f"\n⏸️  Context transition cooling: {THERMAL_CONFIG['context_delay']}s")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python phi4_ud_adaptive_validation.py <context_size>")
        print("Example: python phi4_ud_adaptive_validation.py 12000")
        sys.exit(1)
