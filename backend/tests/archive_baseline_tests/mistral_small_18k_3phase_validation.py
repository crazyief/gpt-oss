"""
phi-4-reasoning @ 18k Context - 3-Phase Validation
...
MODEL_NAME = "Mistral-Small-24B-Instruct-2501-Q6_K (18k context)"
...
def phase1_coarse_discovery():
    ...
    print(f"Item Range: 50 -> 900 (step=50)")
    print(f"Runs per item count: 3")
    print(f"Positions tested: first, middle, last")
    print(f"Estimated queries: {21 * 3 * 3} = 162")
    print(f"Estimated time: ~8-9 minutes")
    print("=" * 80)
    print()
    ...
    for n_items in range(50, 1101, 50):
    ...
    output_file = f"mistral_small_18k_phase1_{timestamp}.json"

    with open(output_file, "w", encoding="utf-8") as f:
        json.dump({
            "model": MODEL_NAME,
            "context_window": "18k tokens",
            "phase": 1,
            "parameters": {
                "start": 50,
                "end": 900,
...
def analyze_phase1_results(results: list):
    ...
    print("-" * 80)
    if all_perfect:
        print("[OK] All tests passed! Safe zone confirmed (50-1050 items).")
    else:
...
def phase3_production_validation():
    ...
    - Test critical boundaries: 50, 400, 800 items
    - 5 runs per item count (95% confidence)
    """
    print("\n" + "=" * 80)
    print("PHASE 3: PRODUCTION VALIDATION")
    print("=" * 80)
    print(f"Validation points: 50, 400, 800 items")
    ...
    # Save results
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = f"mistral_small_18k_phase3_{timestamp}.json"
...
def generate_markdown_report(phase1_results, phase3_results):
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = f"mistral_small_18k_validation_results_{timestamp}.md"

    with open(output_file, "w", encoding="utf-8") as f:
        f.write(f"# phi-4-reasoning 18k Context Validation Report\n\n")
        f.write(f"**Model**: {MODEL_NAME}\n")
        f.write(f"**Context Window**: 18,000 tokens (~1000 items)\n")
        ...
        # Phase 1 Summary
        f.write("## Phase 1: Coarse Discovery (50-1050 items)\n\n")
        ...
        if all_perfect:
            ...
            f.write("- Safe zone confirmed: 50-1050 items\n")
            f.write("- 100% accuracy across all positions\n")
            f.write("- Production-ready with 18k context configuration\n")
        else:
...
def run_full_3phase_validation():
    ...
    print("\n" + "=" * 80)
    print(f"PHI-4-REASONING 16K CONTEXT - 3-PHASE VALIDATION")
    print("=" * 80)
    print(f"Model: {MODEL_NAME}")
    print(f"Context Window: 18,000 tokens")
    print(f"Safe Zone Target: 50-1050 items")
